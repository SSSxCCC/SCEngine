...
struct GLRenderPoints {
	...
	void Flush() {
		...
		m_shader.use();
		glm::mat4 proj = mGameWorld->mMainCamera->buildProjectionMatrix();
		m_shader.setMat4("projectionMatrix", glm::value_ptr(proj));
...
void DebugDraw::Create(std::shared_ptr<GameWorld> gameWorld) {
	mGameWorld = gameWorld;
	m_points = new GLRenderPoints;
	m_points->Create(gameWorld);
	m_lines = new GLRenderLines;
	m_lines->Create(gameWorld);
	m_triangles = new GLRenderTriangles;
	m_triangles->Create(gameWorld);
}
